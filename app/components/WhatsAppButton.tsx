...
import { WHATSAPP_DEFAULT_MESSAGES, getWhatsAppUrl } from "../../lib/whatsapp";
...
export default function WhatsAppButton() {
    ...
    const message = WHATSAPP_DEFAULT_MESSAGES[locale];
    const url = getWhatsAppUrl(message);
    const label =
          locale === "en" ? "Chat with us on WhatsApp" : "Discuter avec nous sur WhatsApp";
  ...
  return h(
        "a",
    {
            ...
            title: label,
            className:
                      "whatsapp-float-btn fixed bottom-6 right-6 z-40 flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg transition hover:scale-105 focus:outline focus:outline-2 focus:outline-offset-2 focus:outline-gold md:bottom-8 md:right-8",
    },
        h(
                "svg",
          {
